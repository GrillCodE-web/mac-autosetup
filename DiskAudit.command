...
# ============================================================
#  DISK AUDIT — АУДИТ ДИСКА И СИСТЕМЫ — v3.1
#  v3.1: в конце по Enter экран и скроллбэк терминала стираются.
...
#  НИЧЕГО не пишет в файлы: отчёт живёт только в окне терминала.
...
#  В конце: выдели весь вывод (Cmd+A, Cmd+C), скинь разработчику,
#  нажми Enter — экран и прокрутка будут стёрты.
# ============================================================
...
echo ""
echo -e "${GREEN}${BOLD}АУДИТ ЗАВЕРШЁН — ничего не изменено, в файлы ничего не записано.${NC}"
echo -e "${GREY}Отчёт существует только в этом окне терминала — на диске его нет.${NC}"
echo ""
echo "  1) Скопируй: Cmd+A (выделить всё), Cmd+C (скопировать) — и скинь разработчику."
echo "  2) Убедись, что вставилось (Cmd+V в любом чате)."
echo ""
read -r -p "  3) Скопировал? Нажми Enter — экран и прокрутка будут стёрты."
# Стираем видимое и скроллбэк: ESC[3J чистит прокрутку в Terminal.app и iTerm2.
clear
printf '\033[3J'
echo -e "${GREEN}✓ Экран и прокрутка стёрты. Окно можно закрыть.${NC}"
echo -e "${GREY}Если запускал командой 'bash DiskAudit.command' руками — удали её из истории: history -d \$(history 1 | awk '{print \$1}')${NC}"
sleep 3
clear
printf '\033[3J'
