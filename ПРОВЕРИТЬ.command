...
# Safari скриптом больше НЕ переносится (v11): cookies и настройки Safari
# всё равно всегда остаются в системе — выносить на диск один профиль
# (закладки/историю) бессмысленно, а контейнер macOS на внешний диск не
# даёт вынести. Если ссылка профиля осталась с прошлых версий — она
# работает, проверяем честно; новый Mac её не получит.
if [ -L "$HOME/Library/Safari" ]; then
    ...
    check_link "$HOME/Library/Safari" "Safari — профиль (ссылка с прошлых версий)" "$SF_NO_DISK"
else
    dim "Safari — скриптом не переносится (v11): закладки и история лежат в системе, это нормально."
fi
...
if [ -L "$HOME/Library/Containers/com.apple.Safari" ]; then
    ...
    dim "Cookies Safari остаются на Mac всегда — не используй Safari для чувствительного."
