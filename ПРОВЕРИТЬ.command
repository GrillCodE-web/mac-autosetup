...
if [ "$AL" = "1800" ]; then
    good "Автовыход из системы через 30 минут включен."
else
    bad "Автовыход не настроен (Настройки -> Конфиденциальность -> Дополнительно)."
fi
...
if [ -n "$DS" ] && [ "$DS" -le 5 ] 2>/dev/null; then
    good "Дисплей гаснет через $DS мин."
else
    dunno "Выключение дисплея: сейчас «$DS» мин — если больше 5, поправь в Настройки -> Экран блокировки."
fi
